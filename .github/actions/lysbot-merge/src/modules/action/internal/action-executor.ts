...
import type { ActionConfig, ActionResult, CheckResult, EventContext } from './types.js';
import {
...
export class ActionExecutor {
  ...
  async execute(context: EventContext): Promise<ActionResult> {
    const {
      owner,
      repo,
      prNumber,
    ...
    let prData = await this.githubClient.fetchPullRequestData(owner, repo, prNumber);
    ...
    if (approvalOverridden) {
      ...
      this.core.info('Approval requirement overridden by command flag (--override-approval-requirement).');
    }
    ...
    const mergeMethodResult = determineMergeMethod(prData.headRef, prData.baseRef, this.config);
    ...
    prData = await this.githubClient.fetchPullRequestData(owner, repo, prNumber);
    ...
    let retries = 0;
    while (prData.mergeable === null && retries < this.config.mergeableRetryCount) {
      await waitBeforeRetryMs(this.config.mergeableRetryInterval * 1000);
      prData = await this.githubClient.fetchPullRequestData(owner, repo, prNumber);
      retries++;
