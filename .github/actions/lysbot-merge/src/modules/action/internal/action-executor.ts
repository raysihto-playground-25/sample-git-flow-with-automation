...
export class DefaultActionExecutor implements ActionExecutor {
  constructor(
    ...
    private readonly prValidator: PrValidator,
    private readonly markdownBuilder: MarkdownBuilder,
    private readonly waitService: WaitService,
  ) {}

  async executeAction(context: EventContext, config: ActionConfig): Promise<ActionResult> {
    const {
      owner,
      repo,
      prNumber,
    ...
    if (this.commandParser.isBot(userType)) {
      return { status: 'skipped', message: 'Comment is from a bot' };
    }

    const mergeOptions = this.commandParser.parseCommand(commentBody);
    if (!mergeOptions) {
    ...
    if (!this.commandParser.hasValidAuthorAssociation(authorAssociation)) {
    ...
    if (!this.commandParser.hasValidPermission(permission)) {
      await this.githubClient.postComment(
        owner,
        repo,
        prNumber,
    ...
    let prData = await this.pullRequestService.fetchPullRequestData(owner, repo, prNumber);
    ...
    const prStateChecks = this.prValidator.validatePRState(prData);

    const unresolvedCount = await this.pullRequestService.countUnresolvedThreads(owner, repo, prNumber);
    ...
    const approvedReviews = await this.pullRequestService.fetchApprovedReviews(owner, repo, prNumber);
    ...
    for (const review of approvedReviews) {
      ...
      if (review.commit_id !== prData.headSha) {
        ...
        const dismissed = await this.pullRequestService.dismissReview(owner, repo, prNumber, review.id, message);
        if (!dismissed) {
    ...
    const conflictsCheck: CheckResult = {
      ...
      passed: noConflicts,
      ...(!noConflicts && { details: this.prValidator.getMergeableStateDescription(prData.mergeableState) }),
    };

    const isConventionalTitle = this.prValidator.isConventionalCommitTitle(prData.title);
    ...
    const mergeMethodResult = this.prValidator.determineMergeMethod(prData.headRef, prData.baseRef, config);
    ...
    const checksMarkdown = this.markdownBuilder.buildCheckResultsMarkdown(checks);
    ...
    prData = await this.pullRequestService.fetchPullRequestData(owner, repo, prNumber);
    ...
    while (prData.mergeable === null && retries < config.mergeableRetryCount) {
      await this.waitService.waitBeforeRetryMs(config.mergeableRetryInterval * 1000);
      prData = await this.pullRequestService.fetchPullRequestData(owner, repo, prNumber);
      retries++;
    ...
    if (mergeMethodResult.method === 'merge') {
      ...
      const commits = await this.pullRequestService.fetchPullRequestCommits(owner, repo, prNumber);
    ...
    const mergeResult = await this.pullRequestService.mergePullRequest(
      owner,
      repo,
      prNumber,
