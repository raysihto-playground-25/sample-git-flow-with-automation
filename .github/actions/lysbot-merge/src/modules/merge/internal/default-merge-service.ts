...
export class DefaultMergeService implements MergeService {
  async executeAction(octokit: Octokit, context: EventContext, config: ActionConfig): Promise<ActionResult> {
    ...
    if (validators.isBot(userType)) {
      return { status: 'skipped', message: 'Comment is from a bot' };
    }

    const mergeOptions = validators.parseCommand(commentBody);
    if (!mergeOptions) {
      return { status: 'skipped', message: 'Command not matched' };
    }

    await githubClient.addReaction(octokit, owner, repo, commentId, 'eyes');

    if (!validators.hasValidAuthorAssociation(authorAssociation)) {
    ...
    const permission = await githubClient.getCollaboratorPermission(octokit, owner, repo, actor);
    if (!validators.hasValidPermission(permission)) {
      await githubClient.postComment(
        octokit,
        owner,
        repo,
    ...
    let prData = await githubClient.fetchPullRequestData(octokit, owner, repo, prNumber);
    ...
    const prStateChecks = validators.validatePRState(prData);

    const unresolvedCount = await githubClient.countUnresolvedThreads(octokit, owner, repo, prNumber);
    ...
    const approvedReviews = await githubClient.fetchApprovedReviews(octokit, owner, repo, prNumber);
    ...
    for (const review of approvedReviews) {
      ...
      if (review.commit_id !== prData.headSha) {
        ...
        const dismissed = await githubClient.dismissReview(octokit, owner, repo, prNumber, review.id, message);
        if (!dismissed) {
    ...
    if (dismissFailures.length > 0) {
      ...
      await githubClient.postComment(octokit, owner, repo, prNumber, staleComment);
    }
    ...
    const conflictsCheck: CheckResult = {
      ...
      passed: noConflicts,
      ...(!noConflicts && { details: validators.getMergeableStateDescription(prData.mergeableState) }),
    };

    const isConventionalTitle = validators.isConventionalCommitTitle(prData.title);
    ...
    const mergeMethodResult = validators.determineMergeMethod(prData.headRef, prData.baseRef, config);
    ...
    const checksMarkdown = formatters.buildCheckResultsMarkdown(checks);
    ...
    prData = await githubClient.fetchPullRequestData(octokit, owner, repo, prNumber);
    ...
    while (prData.mergeable === null && retries < config.mergeableRetryCount) {
      await formatters.waitBeforeRetryMs(config.mergeableRetryInterval * 1000);
      prData = await githubClient.fetchPullRequestData(octokit, owner, repo, prNumber);
      retries++;
    ...
    if (prData.mergeable === false || prData.mergeable === null || prData.mergeableState === 'dirty') {
      ...
      await githubClient.postComment(octokit, owner, repo, prNumber, errorComment);
    ...
    if (mergeMethodResult.method === 'merge') {
      ...
      const commits = await githubClient.fetchPullRequestCommits(octokit, owner, repo, prNumber);
