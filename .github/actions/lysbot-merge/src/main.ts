...
import type { ActionConfig, EventContext } from './modules/action/index.js';
...
export async function run(): Promise<void> {
  try {
    ...
    const actionRunner = configureActionModule();

    const result = await actionRunner.executeAction(octokit, context, config);
    ...
    const summaryMarkdown = actionRunner.buildSummaryMarkdown(
      resultEmoji,
      context.prNumber,
      context.actor,
      result.mergeMethod,
