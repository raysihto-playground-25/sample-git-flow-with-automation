...
const mockConfigureMergeModule = vi.fn().mockReturnValue({
...
vi.mock('../src/modules/merge/index.js', () => ({
  configureMergeModule: mockConfigureMergeModule,
...
describe('main.ts', () => {
  beforeEach(() => {
    ...
    mockMergeService.executeAction.mockResolvedValue({
      status: 'merged',
  ...
  describe('run()', () => {
    it('should successfully execute with default configuration', async () => {
      ...
      expect(mockConfigureMergeModule).toHaveBeenCalled();

      expect(mockMergeService.executeAction).toHaveBeenCalled();
    ...
    it('should handle custom configuration inputs', async () => {
      ...
      expect(mockMergeService.executeAction).toHaveBeenCalledWith(
        expect.any(Object),
        expect.any(Object),
    ...
    it('should handle errors from executeAction', async () => {
      mockMergeService.executeAction.mockRejectedValue(new Error('API error'));
    ...
    it('should handle non-Error exceptions', async () => {
      mockMergeService.executeAction.mockRejectedValue('string error');
